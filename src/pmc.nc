...
static void pmc_show(struct ptp_message *msg, FILE *fp)
{
	...
	char source_pid[64], parent_pid[64], port_pid[64];
    pid2str(source_pid, 64, &msg->header.sourcePortIdentity);
    
	fprintf(fp, "\t%s seq %hu %s ", source_pid, msg->header.sequenceId, action_string[action]);
	if (msg->tlv_count != 1) {
		goto out;
	}
	...
	case TLV_PARENT_DATA_SET:
        pid2str(parent_pid, 64, &pds->parentPortIdentity);
	...
	case TLV_PORT_DATA_SET:
		...
		pid2str(port_pid, 64, &p->portIdentity);
