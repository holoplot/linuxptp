			   ...
			   int excluded)
{
	...
	case TLV_PORT_DATA_SET:
		...
		if (!port) {
            char unknown_pid[64];
            pid2str(unknown_pid, 64, &pds->portIdentity);
			pr_info("received data for unknown port %s", unknown_pid);
			return 1;
		}
		state = normalize_state(pds->portState);
		if (port->state != state) {
            char pds_pid[64];
            pid2str(pds_pid, 64, &pds->portIdentity);
			pr_info("port %s changed state", pds_pid);
...
int phc2sys_main(int argc, char *argv[])
{
	char *progname;
